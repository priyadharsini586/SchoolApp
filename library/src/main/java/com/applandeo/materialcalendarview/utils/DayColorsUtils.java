...
public class DayColorsUtils {
    ...
    public static void setCurrentMonthDayColors(Calendar day, Calendar today, TextView dayLabel,
        ...
        List<EventDay>getEvent = calendarProperties.getEventDays();
        boolean isEvent;
        if (getEvent.size() != 0) {
           isEvent = getEventList(calendarProperties, day);
           if (isEvent)
           {
               dayLabel.setBackgroundResource(R.drawable.select_event_bg);
               dayLabel.setTextColor(Color.parseColor("#000000"));
           }
        }
    }
    public static boolean getEventList(CalendarProperties calendarProperties,Calendar day){
        boolean isEventDate = false;
        List<EventDay>getEvent = calendarProperties.getEventDays();
        DateFormat sdf = new SimpleDateFormat("yyyy/MM/dd");
        String date = sdf.format(day.getTime());
        for (int i =0 ; i < getEvent.size() ; i++){
            EventDay eventDay = getEvent.get(i);
            Calendar getEventCalendar = eventDay.getCalendar();
            String eventDate = sdf.format(getEventCalendar.getTime());
